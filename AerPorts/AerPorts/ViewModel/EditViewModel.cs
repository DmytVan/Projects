...
namespace AerPorts.ViewModel
{
    public class EditViewModel : DependencyObject
    {
            ...
            get
            {
                return editCommand ??
                    (editCommand = new RelayCommand(obj =>
                    {
                        ...
                        {
                            FlightsViewModel.myWindowEd.Close();
                            DBConnection.Update(flight);
                            DBConnection.Select();
                        }
                    }));
            }
        ...
        {
            flight = FlightsViewModel.selectFlight;
        }
        ...
        public static readonly DependencyProperty MyPropertyProperty =
           DependencyProperty.Register("MyProperty", typeof(Flight), typeof(EditViewModel), new PropertyMetadata(null));
        ...
        public static readonly DependencyProperty errorProperty =
            DependencyProperty.Register("error", typeof(string), typeof(EditViewModel), new PropertyMetadata(""));
        ...
        public bool Validation()
        {
            ...
            D = new DateTime(flight.yearD, flight.monthD, flight.yearD, flight.hourD, flight.minD, 0);
            A = new DateTime(flight.yearA, flight.monthA, flight.yearA, flight.hourA, flight.minA, 0);
            if(A<D)
            {
            ...
            if ((A-D).Days>30)
            {
                var res = MessageBox.Show("Полёт составляет " + (A - D).Days + " дней.\n Всё равно добавить?", "", buttons,
            MessageBoxImage.Question);
                if (res == MessageBoxResult.No)
                {
                    return false;
